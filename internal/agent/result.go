...
// It is the source of truth for how the Manager routes the result — never
// branch on the error field alone.
type Status string
...
//	    CompletedAt: time.Now(),
//	}
type AgentResult struct { //nolint:revive // AgentResult is intentionally explicit; callers write agent.AgentResult for clarity
	...
	// Required for StatusSuccess and StatusNeedsHuman; 1-2 sentences.
	...
	// Error contains the error message when Status is StatusFailed.
	// Include enough context for debugging (what failed, why).
...
// Both StatusSuccess and StatusReadyToSubmit are considered successful outcomes.
func (r *AgentResult) IsSuccess() bool {
	return r.Status == StatusSuccess || r.Status == StatusReadyToSubmit
}

// IsFailed returns true if the agent failed and cannot proceed.
func (r *AgentResult) IsFailed() bool {
	return r.Status == StatusFailed
}

// NeedsHuman returns true if the agent requires human intervention before proceeding.
func (r *AgentResult) NeedsHuman() bool {
	return r.Status == StatusNeedsHuman
}

// IsTerminal returns true if the result represents a state that requires no
// further automated action. Terminal states are: success, failed, ready_to_submit.
// StatusNeedsHuman is NOT terminal — the agent must be re-run after human intervention.
// The Manager uses this to decide whether to stop scheduling follow-up agents.
