...
func TestReview_Compliance_NoDocuments_SkipsChecker(t *testing.T) {
	mc := &mockChecker{resp: `{"findings":[]}`}
	a := finalreview.NewWithComplianceChecker(mc)
	...
	if mc.calls != 0 {
		t.Errorf("checker should not run without documents, got %d calls", mc.calls)
	}
	if res.Status != agent.StatusReadyToSubmit {
		t.Errorf("Status = %s, want ready_to_submit", res.Status)
	}
}
