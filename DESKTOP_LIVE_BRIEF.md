...
- [~] Phase 4 frontend rewire — api.js seam DONE (live wrappers getProposals/getWorkspace/pursue/approveProposal/requestChanges/submitProposal/updateSection/draftMarkdown + browser mock fallback; vite build clean; commit e30e505). Malik's call 2026-06-11: PROCEED with the component rewire autonomously (he verifies the live flow by running the real Wails app at PR time; browser can't exercise window.go). NEXT: wire App.jsx (load proposals via getProposals + reload after actions; gate handlers call api; guard the offline simulation for live), workspace.jsx (consume getWorkspace view-model), editor.jsx, submitted.jsx; B3 Download draft via draftMarkdown; B4 confirmation toast. Keep mock fallback so vite dev renders.
