...
- **Store interface** — Abstraction for persistence (`Save(opp)`, `Load(id)`, `List()`). The deployed pipeline runs on the JSON file implementation (`internal/store/json.go`); the interface allows an optional swap to Firestore later without touching agent code. This is the "provision lazily, design eagerly" pattern in action.
